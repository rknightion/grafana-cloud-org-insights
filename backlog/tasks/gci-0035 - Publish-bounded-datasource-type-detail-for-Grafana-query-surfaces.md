...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Top-N view is bounded and scoped to exact instance_id on every query
- [x] #2 Dashboard explains that datasource type is inference, not app identity
- [x] #3 Raw datasource or plugin values never become metric labels
<!-- AC:END -->
...
Code acceptance: exact instance_id query guard, bounded top-20 and remainder per stack, inference banner, zero new metrics or raw-value labels; Gate G and hosted CI 35935370824 passed at product 51ce670. Live dev datasource-type source evidence exists, but the new S3 view was not published because T2 failed before scan and dev was rolled back. Positive panelPluginId proof remains unavailable.
