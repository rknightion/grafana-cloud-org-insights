...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Panels read grafanacloud-usage only; no collector source, no credential, no emitted series
- [x] #2 Integration technology breakdown renders from the integration label with no maintained list
- [x] #3 Named service and owning team render from the OnCall metric labels
- [x] #4 Every rate-shaped expression is windowed and every ratio denominator is population-matched
- [x] #5 Stack naming uses the group_left join, never an arithmetic add
- [x] #6 tests/test_dashboards.py coverage gates pass with the new panels
<!-- AC:END -->
